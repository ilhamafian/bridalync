...
function formatApiError(error: unknown, fallback: string) {
  if (typeof error === "string" && error.trim()) {
    // ZodError.message is a JSON dump of issues — surface the readable message.
    if (error.trimStart().startsWith("[")) {
      try {
        const issues = JSON.parse(error) as Array<{ message?: string }>;
...
export default function AdminLoginPage() {
  ...
  async function sendLoginCode() {
    ...
    if (!response.ok) {
      setError(formatApiError(data.error, "Could not send code."));
      return false;
    }
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(formatApiError(data.error, "Could not sign in."));
        return;
      }
